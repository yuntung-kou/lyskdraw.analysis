...
function addRecord() {
    ...
    let judgeResult = 'wai_std';
    if (isUpCard) {
        if (sub === '混池' && oshis.length > 0 && !oshis.includes(pulledLead)) {
            judgeResult = 'wai_lim';
        } else {
            judgeResult = 'target';
        }
...
window.updateLuckStats = function() {
    ...
    if (mainFilter !== '綜合') {
        mainItems = mainItems.filter(r => r.main === mainFilter);
    }
    document.getElementById('luckMainVal').innerHTML = calcOverall(mainItems);

    let subItems = db.filter(r => r.res === 'target' && r.sub === subFilter);
...
function renderUI() {
    ...
    if (db.length > 0) {
        ...
        document.getElementById('peakBoard').innerHTML = peakHTML || '<div style="text-align:center;font-size:12px;color:var(--text-sub)">尚無目標數據</div>';
    } else {
        document.getElementById('peakBoard').innerHTML = '<div style="text-align:center;font-size:12px;color:var(--text-sub)">尚無資料</div>';
    }
    ...
    document.getElementById('recordList').innerHTML = db.map(r => {
        ...
        let luckBgStyle = 'rgba(0,0,0,0.3)';
        if (r.pulls <= 55) {
            luckBgStyle = r.luck.c + 'BF'; 
        }
        ...
        <div class="h-record-card">
            ...
            <div class="h-content">
                ...
                <div class="h-right">
                    ...
                    <div class="h-luck" style="background:${luckBgStyle};">${r.luck.t}</div>
